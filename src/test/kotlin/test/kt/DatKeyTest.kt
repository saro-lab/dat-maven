...
class DatKeyTest {
    fun unit(failKey: DatKey, signatureAlgorithm: SignatureAlgorithm, cryptoAlgorithm: CryptoAlgorithm) {
        ...
        val newKeyStr = newKey.format(SignatureKeyOutOption.FULL)

        val readKey = parse(newKeyStr, Kid.BY_STRING)

        val dat = newKey.toDat(plain, secure)
        println("$tag: $dat")

        val payload = readKey.toPayload(dat, Kid.BY_STRING)
        println(tag + ": " + payload.plain + " / " + payload.secure)

        assert(plain == payload.plain)
        assert(secure == payload.secure)
        Assertions.assertThrows(Exception::class.java, { failKey.toPayload(dat, Kid.BY_STRING) })
    }


    fun generate(signatureAlgorithm: SignatureAlgorithm, cryptoAlgorithm: CryptoAlgorithm): DatKey {
        return generate(
            Kid.BY_STRING.toKid(generateRandomBase62(10)),
            signatureAlgorithm,
            cryptoAlgorithm,
